...
return (function()
            ...
            { self.Guidance, "Where's the hospital? What I should do?",
                function()
                    if not self.LupusAware then
                        ...
                        Say("Thiago","Hey I need to see a doctor", 3.0)
                        Ada.StartTalk()
                        Say("Ada","Oh yes, You'll need to go to the town's Hospital", 5.0)
                        Say("Ada","You can take the path to the town that's aside our house front", 5.0)
                        Ada.StopTalk()
                        SetEmotion("TPeace")
                        Say("Thiago","Oh, I'm not sure about going alone", 5.0)
                        ...
                        SetEmotion("TNeutral")
                        Say("Ada","Your key is in that table,\nRemember to close the door when leaving", 6.0)
                        self.LupusAware = true
                        SetState("MainKey", "MainKeyBase")
                        PickUp("MainKey",2)
                    end
                    Ada.StartTalk()
                    Say("Ada","So You'll need to take the path aside the house", 4.0)
                    Say("Ada","it'll left you right into the town's Hospital", 4.0)
                    Say("Ada","I know You can do it, The place is small and pretty close", 5.0)
                    Ada.StopTalk()
                    Say("Thiago","Ok..", 3.0)
                    SetClickable("Ada")
        ...
        Choice(
            ...
            { "Do you have sisters?",
                function()
                    ...
                    SetEmotion("TComplicity")
                    Say("Ada","But beware of that crazy fox", 3.0)
                    SetEmotion("TNeutral")
                    Ada.StopTalk()
                    self.TalkAda()
                end },
    ...
    function self.TalkAboutMansion()
        ...
        Ada.StartTalk()
        Say("Ada","Yeah Well I believe the residence is like almost 100 years old", 4)
        ...
        Ada.StopTalk()
        Schedule(1.5, "SetEmotion", "TNeutral")
